define('ember-tri-state/components/tri-state', ['exports', 'ember', 'ember-tri-state/templates/components/tri-state'], function (exports, _ember, _emberTriStateTemplatesComponentsTriState) {
  ...
  var PromiseObject = EmberObject.extend(PromiseProxyMixin);
  ...
  exports['default'] = Component.extend({
    layout: _emberTriStateTemplatesComponentsTriState['default'],
    tagName: '',
    ...
    triState: computed('promise.isPending', function () {
      var canShowLastSuccessful = this.get('showLastSuccessful') && this.get('_lastResolvedData');
      var willRenderLoadingComponent = this.get('promise.isPending') && !canShowLastSuccessful;
      var willRenderErrorComponent = this.get('promise.isRejected') && !canShowLastSuccessful && !this.get('forceResolveAll');
      var willRenderSuccessComponent = this.get('promise.isFulfilled') || this.get('promise.isPending') && canShowLastSuccessful;
     ...
     * @private
     */
    _resolvePromises: function _resolvePromises(promises) {
    ...
    didReceiveAttrs: function didReceiveAttrs() {
       ...
       * @default noop
       */
      this.onRejected = this.getWithDefault('onRejected', function () {});
       ...
       * @default noop
       */
      this.onSettled = this.getWithDefault('onSettled', function () {});

      // Wrap `promises` into a single promise
      var promise = this._resolvePromises(this.get('promises'));

      // Create a promise proxy object that is state aware
      this.set('promise', PromiseObject.create({ promise: promise }));

      // Resolve promise
      this.get('promise').then(function (data) {
        // cache successful data
        _this._lastResolvedData = data;
        _this.onFulfilledData(data);
      })['catch'](this.onRejected)['finally'](this.onSettled);
    }
  });
});